...
if __name__ == '__main__':
    ...
    users_perceptibility = alg.createPerceptibility(ratings_u50, movies_label, file_path)
    ...
    for q in range(5, 55, 5):
        ...
        # 图表示学习：特征矩阵编码
        featureMatrix_data = Utils.embeddings(ratings_u50, objects_quality, users_dfa, L)

        # 样本量少则采用过采样，样本量大则采用下采样
        if len(users_HPU) < 5000:
            sampleX, labelY = Utils.oversampling(L, df, featureMatrix_data, label, objects_quality, ratings_u50, users_dfa)
        else:
            sampleX, labelY = Utils.undersampling(L, df, objects_quality, ratings_u50, users_dfa)
        ...
        PCNNList.to_csv(file_path + '/rs20231004_epochs_' + str(q) + '.csv', index=False, encoding="utf_8_sig")
