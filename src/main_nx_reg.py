...
if __name__ == '__main__':
    ...
    a = []
    for i in range(0, len(users_P), 10):  ##每隔20行取数据
        a.append(i)
    file = users_P.iloc[a]
    ...
    ratings = ratings_u50[ratings_u50.user_id.isin(np.array(users_HPU['user_id']))]
    for l in range(24, 68, 4):
        PCNNList = list()
        pcnnList = list()
        print('parameter L:', l)
        movieLen_data = Utils.embeddings(ratings, Rquality_m, users_DFA, l)
        ...
        for inx, v in enumerate(label.values()):
            torch_label[inx, :] = v
        xtrain1, xtest1, ytrain1, ytest1 = train_test_split(torch_set, torch_label, test_size=0.4, random_state=seed)
        ...
        PCNNList.to_csv('../data/moviesData/result/BiNCNN1028_reg_nx_' + str(l) + '.csv', index=False, encoding="utf_8_sig")
