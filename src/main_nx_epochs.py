            ...
            for i in range(1):  # 200
                ...
                for j in range(5):
                    ...
                    pcnnList.append([precision_score(labels2, predicted2), recall_score(labels2, predicted2),
                                     f1_score(labels2, predicted2)])
                    print('pcnn>>>>', precision_score(labels2, predicted2), recall_score(labels2, predicted2),
                          f1_score(labels2, predicted2))

            pcnnList = pd.DataFrame(pcnnList, columns=['precision', 'recall', 'f1'])
            PCNNList.append([l, pcnnList['precision'].mean(), pcnnList['recall'].mean(), pcnnList['f1'].mean()])
        PCNNList = pd.DataFrame(PCNNList, columns=['l', 'precision', 'recall', 'f1'])
        PCNNList.to_csv('../data/moviesData/result/PCNNdata0929_' + str(q) + '.csv', index=False, encoding="utf_8_sig")
